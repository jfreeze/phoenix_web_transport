...
if Code.ensure_loaded?(:cowboy_webtransport) do
  defmodule PhoenixWebTransport.Cowboy.Handler do
    @moduledoc """
    Optional cowboy backend: a `cowboy_webtransport` handler that drives a
    `Phoenix.Socket.Transport` (such as `Phoenix.LiveView.Socket`) over a
    WebTransport session. See `PhoenixWebTransport.Quic.Session` for the
    default erlang_quic backend.
