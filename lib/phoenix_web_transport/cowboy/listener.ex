# Compiled only when cowboy (with WebTransport) is a dependency.
if Code.ensure_loaded?(:cowboy_webtransport) do
  defmodule PhoenixWebTransport.Cowboy.Listener do
    @moduledoc """
    Optional backend: a cowboy HTTP/3 (QUIC) listener that only serves
    WebTransport sessions for one Phoenix socket. The default backend is
    `PhoenixWebTransport.Quic.Listener` (erlang_quic, no native build); this
    one needs `cowboy` and `quicer` as dependencies plus `mix deps.quic`.
    ...
    ## Options
      ...
      * `:stream_priority` - `:peek` (default) or `:command` (patched cowboy), see `PhoenixWebTransport.Cowboy.Handler`
    ...
    def init(opts) do
      ...
      dispatch =
        :cowboy_router.compile([
          {:_,
           [
             {~c"#{path}/[...]", PhoenixWebTransport.Cowboy.Handler,
              {endpoint, socket, socket_opts}}
           ]}
        ])
